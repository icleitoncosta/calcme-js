...
export const DIR_BASE = path.resolve(__dirname, '../../src/calcme');
const dirs = [''];
...
async function start() {
  ...
  const result = await page.evaluate((dirs: string[]) => {
    ...
    const modules: any = window.MCALC.calcme as any;
    ...
    for (const dir of dirs) {
      ...
      for (const name of Object.keys(submodules)) {
        ...
        result[resultName] = window.MCALC.calcme._moduleIdMap.get(module);
      }
    }
